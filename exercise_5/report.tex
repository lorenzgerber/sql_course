...
\item define Schemes\\
  ...
  $R_2\{A, D, E, F\}: \fdep{DE}{A},\ \fdep{DE}{F},\ \fdep{F}{E}$\\
...
\subsection*{f) show that 3NF normalization from d) is acyclic/fully independent}

  
\section*{Problem 2 - BCNF Normalization}
Yes can. not cyclic.
\Tree [.{$ABCDEF$\\$\fdep{DE}{F}$} [.{$DEF$\\$\fdep{DE}{F}$} ] [.{$ABCDE$\\$\fdep{CD}{E}$}  [.{$ABCD$\\$\fdep{AB}{C}$} [.{$ABC$\\$\fdep{AB}{C}$} ] [.{$ABD$} ] ] [.{$CDE$\\$\fdep{CD}{E}$} ] ] ]
